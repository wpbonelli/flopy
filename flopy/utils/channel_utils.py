...
def get_mannings_discharge_rect(
...
def get_wetted_area(x0, x1, h0, h1, depth):
    """
    Computes the cross-sectional area of a segment of a channel filled to a given depth.
...
def wetted_area(
    ...
    """
    Computes the cross-sectional area of a channel filled to a given depth.
...
def wetted_perimeter(
    ...
    """
    Computes the total length of the intersection between channel a perimeter and a cross-sectional
    plane normal to the direction of flow. Refer to https://en.wikipedia.org/wiki/Wetted_perimeter.
...
def manningsq(
    ...
    rectangular channel with the provided cross-sectional geometry
    and slope and Manning's roughness coefficient. To convert from
    metric to another measurement system use the conversion factor
    parameter. See https://en.wikipedia.org/wiki/Manning_formula.
...
def qtodepth(
    ...
    """
    Given the flow rate and cross-sectional geometry of a channel, compute the depth of flow.
...
def get_depths(
    ...
    """
    Uses Manning's equation to calculate the depth of flow for a rectangular
    channel given its cross-sectional geometry, a roughness coefficient, the
    flow rate, and the channel's slope. An optional conversion factor can be
    provided, which is used to convert the unit of length from metric system
    another measurement system (e.g. for English units specify factor 1.49).
    An array is returned with the depth of flow for each flow rate provided.

    Parameters
    ----------
    flows: array_like
        The flow rate across the channel
    x: float
        The channel's width
    h: float
        The channel's depth
    roughness: float, optional
        The channel's Manning's roughness coefficient
    slope: float, optional
        The channel's slope
    conv: float, optional
        The conversion factor for converting from metric to another unit
    dd: float, optional
        The depth increment used for calculating the derivative
